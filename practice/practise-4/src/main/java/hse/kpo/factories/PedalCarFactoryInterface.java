...
public class PedalCarFactoryInterface implements CarFactoryInterface<PedalEngineParams> {
    ...
    /**
     * fucntion which make a pedal car.
     *
    ...
    public Car createCar(PedalEngineParams carParams, int carNumber) {
        var engine = new PedalEngineInterface(carParams.pedalSize()); // создаем двигатель
        // на основе переданных параметров

        return new Car(carNumber, engine); // возвращаем собранный автомобиль
        // с установленным двигателем и определенным номером
    }
}
